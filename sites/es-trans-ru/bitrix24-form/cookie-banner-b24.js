/*
 * ⛔ УСТАРЕЛО — НЕ ВЫКЛАДЫВАТЬ. Исторический артефакт сессии 2026-08-12.
 *
 * Файл отстал от прода: нет атрибутов data-lang, текст ссылки — старый
 * («Соглашением об обработке данных» вместо «Согласием на обработку
 * данных»). Выкладка этой версии откатит сайт назад.
 *
 * Актуальный патч для выкладки — ../cookie-banner-i18n-fix/cookie-banner.js
 * Зеркало того, что сейчас на проде — ../cookie-banner/cookie-banner.js
 *
 * Оставлен только ради истории патча Bitrix24 (README.md в этой папке
 * по-прежнему актуален как описание того, КАК делался гейтинг формы).
 *
 * ---
 *
 * Cookie-consent баннер для es-trans.ru — версия с гейтингом
 * Яндекс.Метрики И формы заявки Bitrix24.
 *
 * Это сборка cookie-banner.js (../cookie-banner/cookie-banner.js)
 * с добавленным патчем Bitrix24 (см. README.md в этой папке).
 *
