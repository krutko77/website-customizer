...
(async () => {
  ...
  check(
    /Согласие соискателя/.test(h1.text) && /на обработку персональных данных/.test(h1.text),
  ...
  // ---------- интеграция: подвал ----------
  // Не проверяем как обязательное условие: неизвестно, планируется ли
  // ссылка в общем подвале, или документ используется только рядом с
  // формой отклика на вакансию (см. README). Просто фиксируем в отчёте.
  ...
  console.log(
    footer
      ...
      : '  ℹ В подвале нет ссылки на Согласие соискателя (не проверяется как обязательное — см. README)'
  );
