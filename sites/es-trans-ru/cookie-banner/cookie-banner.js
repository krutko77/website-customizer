 ...
 *
 * ⚠️ ЭТО НЕ ПАТЧ ДЛЯ ВЫКЛАДКИ И БОЛЬШЕ НЕ ЗЕРКАЛО ПРОДА.
 *
 * ⚠️ УСТАРЕЛ с 2026-08-18 12:48 GMT. С этого момента на проде работает
 * версия из ../consent-gating-fix/ (гейтинг Яндекс.Карты через заглушку +
 * отложенная загрузка формы Битрикс24 + i18n-словарь). Этот файл её НЕ
 * содержит и как точка отсчёта уже не годится.
 *
 * Актуальный исходник, совпадающий с продом, — ../consent-gating-fix/
 * cookie-banner.js. Именно оттуда брать файл для деплоя и с ним же
 * сверяться. Патч ../cookie-banner-i18n-fix/ выложен ранее и поглощён
 * consent-gating-fix — самостоятельного значения не имеет.
 *
 * Сверено с продом 2026-08-18 (app.min.js, last-modified 09:32 GMT) —
 * т.е. ДО выкладки consent-gating-fix.
 *
...
(function () {
	...
	function buildBanner() {
		...
		// русским. Это уже починено на проде — см. ../consent-gating-fix/
		// (свой словарь TRANSLATIONS + getLang(): localStorage['language'],
		// затем navigator.language, затем 'ru').
		banner.innerHTML =
