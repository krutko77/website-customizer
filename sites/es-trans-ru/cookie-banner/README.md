...
> ## ⚠️ Эта папка — ЗЕРКАЛО ПРОДА, а не патч для выкладки
>
> `cookie-banner.js` здесь отражает то, что **уже работает** на
> https://es-trans.ru (сверено 2026-08-18). Класть его в Gulp-проект
> бессмысленно и опасно.
>
> **Какой файл брать для деплоя:**
>
> | Задача | Файл |
> |---|---|
> | Патч, ожидающий выкладки (перевод en/cn) | `../cookie-banner-i18n-fix/cookie-banner.js` |
> | Посмотреть, что сейчас на проде | `cookie-banner.js` (эта папка) |
> | Новый сайт с нуля | `../../cookie-banner-universal/` |
> | ⛔ Не трогать (устарело) | `../bitrix24-form/cookie-banner-b24.js`, `../cookie-settings/` |
>
> **Почему это важно.** 2026-08-18 устаревшая версия из этой папки по
> ошибке попала в сборку и уехала на прод: в ней был `insertBefore`
> вместо `mount.appendChild`, из-за чего форма заявки Bitrix24
> отрисовывалась в `<head>` и была невидима — попап заявки оказался
> пустым, приём заявок остановился, потребовался откат. Файл
> синхронизирован с продом, чтобы это не повторилось.
>
> **Проверка перед любой выкладкой** — открыть файл и убедиться, что в нём
> есть `mount.appendChild(s)` и **нет** `insertBefore`.
>
> ⚠️ **Исключение 09.09.2026:** `cookie-banner.css` в этой папке правился
> вместе с актуальным патчем `../legal-review-2026-09/cookie-banner.css`
> (стиль ссылки — линия под текстом через `::before` вместо
> `text-decoration`) и **на проде ещё не был**, только приведён в
> соответствие для единообразия. Патч на выкладку — файл в
> `legal-review-2026-09/`, см. его README.
